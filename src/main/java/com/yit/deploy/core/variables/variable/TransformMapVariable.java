...
public class TransformMapVariable<S, T> extends MapVariable<T> {
    private final MapVariable<S> variable;
    private final ClosureWrapper<T> closure;

    public TransformMapVariable(MapVariable<S> variable, ClosureWrapper<T> closure) {
        this(variable, closure, null, null);
    }

    public TransformMapVariable(MapVariable<S> variable, ClosureWrapper<T> closure, VariableName name, String id) {
        this(variable, closure, name, id, null);
    }

    private TransformMapVariable(MapVariable<S> variable, ClosureWrapper<T> closure, VariableName name, String id, ResolveContext context) {
     ...
     */
    @Override
    public TransformMapVariable<S, T> name(@Nonnull VariableName name) {
        if (Objects.equals(this.name, name)) {
            return this;
        }
        return new TransformMapVariable<>(variable, closure, name, id, context);
    }
     ...
     */
    @Override
    public TransformMapVariable<S, T> context(ResolveContext context) {
    ...
    public VariableInfo toInfo() {
        return new VariableInfo(name, id, VariableType.transformMap, variable.toInfo(), closure.toInfo());
    }
    ...
    protected Iterator<Entry<String, Variable<T>>> variables() {
        return Lambda.map(variable.variables(), entry -> new SimpleEntry<>(
            entry.getKey(),
            new TransformVariable<>(entry.getValue(), closure)));
    }
    ...
    public Variable<T> variable(String key) {
        Variable<S> v = variable.variable(key);
        return v == null ? null : new TransformVariable<>(v, closure);
    }
