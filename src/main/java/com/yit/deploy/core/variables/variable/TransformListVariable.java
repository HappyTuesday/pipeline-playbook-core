...
public class TransformListVariable<S, T> extends ListVariable<T> {

    private final ListVariable<S> variable;
    private final ClosureWrapper<T> closure;

    public TransformListVariable(ListVariable<S> variable, ClosureWrapper<T> closure) {
        this(variable, closure, null, null);
    }

    public TransformListVariable(ListVariable<S> variable, ClosureWrapper<T> closure, VariableName name, String id) {
        this(variable, closure, name, id, null);
    }

    private TransformListVariable(ListVariable<S> variable, ClosureWrapper<T> closure, VariableName name, String id, ResolveContext context) {
     ...
     */
    @Override
    public TransformListVariable<S, T> name(@Nonnull VariableName name) {
        if (Objects.equals(this.name, name)) {
            return this;
        }
        return new TransformListVariable<>(variable, closure, name, id, context);
    }
     ...
     */
    @Override
    public TransformListVariable<S, T> context(ResolveContext context) {
    ...
    public VariableInfo toInfo() {
        return new VariableInfo(name, id, VariableType.transformList, variable.toInfo(), closure.toInfo());
    }
    ...
    protected Variable<T> variable(int index) {
        return new TransformVariable<>(variable.variable(index), closure);
    }
    ...
    protected Iterator<Variable<T>> variables() {
        return Lambda.map(variable.variables(), v -> new TransformVariable<>(v, closure));
    }
